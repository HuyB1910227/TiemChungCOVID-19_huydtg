...
@section('breadcrumb')
        ...
        <li class="breadcrumb-item" aria-current="page">Thêm thành viên</li>
...
<div class="card w-50 m-auto shadow">
    <div class="card-body">
        <form action="{{route('admin.member.store')}}" method="post" id="frmQLVC">
            @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="immunization_unit" id="immunization_unit" class="custom-select">
                        ...
                        @foreach ($immunizationUnits as $immunizationUnit)
                            <option value="{{$immunizationUnit->id}}" {{old('immunization_unit') == $immunizationUnit->id ? "selected" : ""}}>{{$immunizationUnit->name}}</option>
                      
                        @endforeach
                    </select>
                    @error('immunization_unit')
                        <div class="error-block mt-1">
                            <strong>Chưa chọn đơn vị tiêm chủng</strong>
                        </div>
                    @enderror
                </div>
                <div class="form-group">
