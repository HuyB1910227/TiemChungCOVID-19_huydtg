...
<div>
    <div class="bg-white card w-75 m-auto p-3">
        <form action="{{route('manager.schedule.prepareUpdate', $schedule->id)}}" method="post" id="">
            @csrf
            <div class="m-auto">
                <h5 class="text-primary">1. Thay đổi lịch tiêm</h5>
                <div class="form-group">
                    <label for="vaccination_date">Ngày tiêm</label>
                    <input type="date" name="vaccination_date" id="vaccination_date" class="form-control" value="{{old('vaccination_date', $schedule->vaccination_date)}}">
                ...
                <div class="form-group">
                    ...
                    <input type="time" name="start_time" id="start_time" class="form-control" value="{{old('start_time', $schedule->start_time)}}">
                    @error('start_time')
                    <div class="error-block mt-1">
                        <strong>{{$message}}</strong>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <input type="time" name="end_time" id="end_time" class="form-control" value="{{old('end_time', $schedule->end_time)}}">
                    @error('end_time')
                    <div class="error-block mt-1">
                        <strong>{{$message}}</strong>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <select name="vaccine" id="vaccine" class="custom-select">
                        ...
                        @foreach ($vaccines as $vaccine)
                            <option value="{{$vaccine->id}}" {{old('vaccine', $schedule->vaccine_id) == $vaccine->id ? "selected" : ""}}>{{$vaccine->name}}</option>
                        @endforeach
                    </select>
                    @error('vaccine')
                    <div class="error-block mt-1">
